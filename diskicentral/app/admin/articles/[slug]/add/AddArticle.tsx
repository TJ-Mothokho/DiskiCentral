...
import {
  MarkdownPreview,
  MarkdownToolbar,
} from "@/components/article/MarkdownEditor";
...
export default function ArticleForm({ article }: ArticleFormProps) {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([
      categoriesService.getApiCategories(),
      tagsService.getApiTags(),
      teamsService.getApiTeams(),
    ])
      .then(([categoryResponse, tagResponse, teamResponse]) => {
        if (!active) return;
      ...
      .catch(() =>
        setError(
          "Some article options could not be loaded. Please refresh and try again.",
        ),
      )
  ...
  async function handleCreateTag() {
    ...
    setCreatingTag(true);
    try {
      const response = await tagsService.addTag({ name, slug: slugify(name) });
      setTags((current) => [...current, response.data]);
      setField("tagIds", [...values.tagIds, response.data.id]);
      setNewTagName("");
    } catch {
    ...
    try {
      ...
      setError(
        typeof submitError === "string"
          ? submitError
          : `Failed to ${editing ? "update" : "create"} article.`,
      );
    } finally {
      setSubmitting(false);
    }
  ...
  function applyLinePrefix(prefix: string) {
    ...
    const nextBody = `${values.body.slice(0, lineStart)}${selected
      .split("\n")
    ...
    updateBody(
      nextBody,
      lineStart,
      lineStart + selected.length + prefix.length * selected.split("\n").length,
    );
  }
      ...
      <form
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label
                htmlFor="body"
                className="block text-xs font-medium text-gray-400">
                Body <span className="ml-1 text-[#00C853]">*</span>
              </label>
              <span className="text-[11px] text-gray-600">
                Markdown is saved with the article
              </span>
            </div>
            <MarkdownToolbar
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              <textarea
                ...
                rows={18}
                className="w-full resize-y rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm leading-6 text-white outline-none focus:border-[#00C853]"
                placeholder="Write with Markdown, or use the formatting toolbar..."
              />
              <MarkdownPreview markdown={values.body} />
            </div>
          </div>
        </div>
        <aside className="space-y-5">
          ...
          <div className="space-y-4 rounded-xl border border-gray-800 bg-[#111] p-5">
            ...
            <div className="space-y-1.5">
              <label
                htmlFor="categoryId"
                className="block text-xs font-medium text-gray-400">
                Category <span className="ml-1 text-[#00C853]">*</span>
              </label>
              <select
                required
                id="categoryId"
            ...
            <div className="space-y-1.5">
              ...
              <select
                id="teamId"
                value={values.teamId ?? ""}
                onChange={(e) => setField("teamId", e.target.value || null)}
                disabled={loadingOptions}
                className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white outline-none focus:border-[#00C853]">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex max-h-32 flex-wrap gap-2 overflow-y-auto">
                {tags.map((tag) => (
                  <label
                    key={tag.id}
                    className={`cursor-pointer rounded-full border px-2.5 py-1 text-xs ${selectedTagNames.has(tag.id) ? "border-[#00C853] bg-[#00C853]/10 text-[#00C853]" : "border-gray-700 text-gray-400"}`}>
                    <input
                      type="checkbox"
                      className="sr-only"
                      checked={selectedTagNames.has(tag.id)}
                      onChange={(e) =>
                        setField(
                          "tagIds",
                          e.target.checked
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-lg border border-gray-700 px-3 py-2 text-xs font-semibold text-gray-300 hover:border-[#00C853] hover:text-[#00C853] disabled:cursor-not-allowed disabled:opacity-50">
                  <Plus size={14} /> Add
                </button>
              </div>
            </div>
          ...
          <ImageField
            label="Hero image"
            preview={heroPreview}
            onChange={(file) => selectFile("heroImage", file)}
          />
          <ImageField
            label="Thumbnail"
            preview={thumbnailPreview}
            onChange={(file) => selectFile("thumbnail", file)}
          />
          {error && (
