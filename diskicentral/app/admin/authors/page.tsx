...
export default function AdminAuthorsPage() {
  ...
  const [formAuthor, setFormAuthor] = useState<Author | null | undefined>(
    undefined,
  );
  ...
  function handleSaved(saved: Author) {
    setAuthors((current) =>
      current.some((author) => author.id === saved.id)
        ? current.map((author) => (author.id === saved.id ? saved : author))
        : [...current, saved],
    );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-lg font-display font-bold text-white">Authors</h1>
          <p className="mt-1 text-xs text-gray-500">
            {loading
              ? "Loading authors..."
              : `${authors.length} author profiles`}
          </p>
        </div>
        <button
          type="button"
          onClick={() => setFormAuthor(null)}
          className="flex items-center gap-1.5 rounded-lg bg-[#00C853] px-4 py-2 text-sm font-bold text-black hover:bg-[#00A344]">
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-800 bg-[#111]">
        <table className="w-full text-sm">
          <thead className="bg-gray-900/50">
            <tr>
              {["Author", "User", "Slug", "Social", "Actions"].map(
                (heading) => (
                  <th
                    key={heading}
          ...
          <tbody>
            {authors.map((author) => {
              const user = users.find((item) => item.id === author.userId);
              return (
                <tr key={author.id} className="border-t border-gray-800">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      {author.avatar ? (
                        ...
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#00C853]/15 text-sm font-bold text-[#00C853]">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        ...
                        className="rounded-lg p-1.5 text-red-400 hover:bg-gray-800 hover:text-red-300 disabled:opacity-50">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
      ...
      {formAuthor !== undefined && (
        <AuthorModal
          author={formAuthor ?? undefined}
          users={users}
          onClose={() => setFormAuthor(undefined)}
          onSaved={handleSaved}
        />
      )}
    </div>
...
function AuthorModal({ author, users, onClose, onSaved }: FormProps) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const saved =
        editing && author
          ? await authorsService.updateAuthor(author.id, {
              name,
              slug,
              bio: bio || null,
              avatar,
              twitter: twitter || null,
            } satisfies UpdateAuthor)
          : await authorsService.addAuthor({
              userId,
              name,
              slug,
              bio: bio || null,
      ...
      setError(
        typeof reason === "string"
          ? reason
          : `Failed to ${editing ? "update" : "add"} author.`,
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div
      ...
      onClick={onClose}>
      <div
        className="w-full max-w-xl rounded-xl border border-gray-800 bg-[#111] p-5"
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          {!editing && (
            <div className="space-y-1.5">
              <label
                ...
                User <span className="ml-1 text-[#00C853]">*</span>
              </label>
              <select
                required
                id="userId"
                ...
                onChange={(event) => {
                  ...
                  setSlug(slugifyName(selectedName));
                }}
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="rounded-lg bg-[#00C853] px-4 py-2 text-sm font-bold text-black hover:bg-[#00A344] disabled:opacity-50">
              {submitting
                ? "Saving..."
                : editing
                  ? "Save Changes"
